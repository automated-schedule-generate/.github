...
for (const pair of (env.MEMBER_MAP || "")
  .split(",")
  .map((s) => s.trim())
  .filter(Boolean)) {
  const [t, g] = pair.split(":").map((x) => x && x.trim());
  if (t && g) {
    t2gMember.set(t.toLowerCase(), g);
...
const sha1 = (o) =>
  crypto
    .createHash("sha1")
    .update(typeof o === "string" ? o : JSON.stringify(o))
    .digest("hex");
...
async function trello(path, { method = "GET", params = {} } = {}) {
  ...
  if (!res.ok)
    throw new Error(
      `Trello ${method} ${path} -> ${res.status} ${await res.text()}`,
    );
...
const DONE_PATTERNS = (
  env.TRELLO_DONE_LIST && env.TRELLO_DONE_LIST.trim()
...
const doneListIds = new Set(
  lists
    .filter((l) =>
      DONE_PATTERNS.some((p) => norm(l.name) === p || norm(l.name).includes(p)),
    )
    .map((l) => l.id),
);
const primaryDoneListId = [...doneListIds][0] || null; // p/ onde mover card quando a issue fecha
const newCardListId =
  (NEW_CARD_LIST_NAME && listByName.get(norm(NEW_CARD_LIST_NAME))) ||
  (lists[0] && lists[0].id);

const boardLabels = await trello(`/boards/${BOARD_ID}/labels`, {
  params: { fields: "name,color", limit: 1000 },
});
const labelById = new Map(boardLabels.map((l) => [l.id, l]));
const labelByName = new Map(
  boardLabels.filter((l) => l.name).map((l) => [l.name.toLowerCase(), l]),
);
...
for (const m of boardMembers) {
  let email = null;
  try {
    const full = await trello(`/members/${m.id}`, {
...
const memberByUsername = new Map(
  [...memberById.values()].map((m) => [m.username.toLowerCase(), m]),
);
...
const allIssues = (
  await ghPaged(`/repos/${OWNER}/${REPO}/issues?state=all`)
).filter((i) => !i.pull_request);
const repoLabels = new Set(
  (await ghPaged(`/repos/${OWNER}/${REPO}/labels`)).map((l) =>
    l.name.toLowerCase(),
  ),
);
...
for (const iss of allIssues) {
  const m = (iss.body || "").match(
    /<!--\s*trello-sync:card=([a-z0-9]+)\s*-->/i,
  );
  if (m) {
    cardToIssue.set(m[1], iss.number);
    issueToCard.set(iss.number, m[1]);
    originOf.set(m[1], "trello");
  }
}
// github-origin: pelo anexo do card apontando para a issue
const issueUrlRe = new RegExp(
  `github\\.com/${OWNER}/${REPO}/issues/(\\d+)`,
  "i",
);
for (const c of cards) {
  ...
  for (const at of c.attachments || []) {
    ...
    if (m) {
      const n = Number(m[1]);
      cardToIssue.set(c.id, n);
      issueToCard.set(n, c.id);
      originOf.set(c.id, "github");
      break;
    }
  }
}
...
async function ensureTrelloLabel(name) {
  ...
  const created = await trello("/labels", {
    method: "POST",
    params: { name, color: "blue", idBoard: BOARD_ID },
  });
  labelById.set(created.id, created);
  labelByName.set(name.toLowerCase(), created);
  return created.id;
}
const cardLabelObjs = (c) =>
  (c.idLabels || []).map((id) => labelById.get(id)).filter((l) => l && l.name);
const cardLabelNames = (c) =>
  cardLabelObjs(c)
    .map((l) => l.name)
    .sort();
...
async function resolveAssignees(card) {
  ...
  for (const mid of card.idMembers || []) {
    const l = await ghLoginForMember(mid);
...
function stripBlock(body) {
  ...
  if (s >= 0 && e >= 0)
    return (body.slice(0, s) + body.slice(e + END_TAG.length)).trim();
  return body.trim();
}

const cardState = (c) =>
  c.closed || doneListIds.has(c.idList) ? "closed" : "open";

// ── Aplicadores ───────────────────────────────────────────────────────────────
async function applyIssueFromCard(iss, card, desired) {
  for (const n of desired.labels)
    await ensureGhLabel(
      n,
      TRELLO_HEX[labelByName.get(n.toLowerCase())?.color] || "ededed",
    );
  ...
  const patch = {
    ...
    state: desired.state,
    labels,
    body: injectBlock(iss.body || "", card),
  };
  if (DRY_RUN) {
    console.log(`   [dry] atualizaria issue #${iss.number}`);
    return;
  }
  try {
    await gh(`/repos/${OWNER}/${REPO}/issues/${iss.number}`, {
      method: "PATCH",
      body: { ...patch, assignees: desired.assignees },
    });
  } catch {
    await gh(`/repos/${OWNER}/${REPO}/issues/${iss.number}`, {
      method: "PATCH",
  ...
  } // responsável não atribuível -> ignora
}
async function applyCardFromIssue(card, desired) {
  const idLabels = [];
  for (const n of desired.labels) {
    const id = await ensureTrelloLabel(n);
    if (id) idLabels.push(id);
  }
  const params = {
    name: desired.title,
    desc: desired.desc.slice(0, 16000),
    idLabels,
  };
  if (desired.state === "closed") {
    if (primaryDoneListId) params.idList = primaryDoneListId;
  } else if (doneListIds.has(card.idList)) {
    params.idList = newCardListId;
  }
  if (DRY_RUN) {
    console.log(`   [dry] atualizaria card ${card.id}`);
    return;
  }
...
function desiredCard(iss) {
  return {
    ...
    labels: (iss.labels || [])
      .map((l) => l.name)
      .filter((n) => n.toLowerCase() !== SYNC_LABEL.toLowerCase())
      .sort(),
...
async function createIssueFromCard(card, state) {
  ...
  if (DRY_RUN) {
    console.log(`   [dry] criaria issue do card "${card.name}"`);
    return;
  }
  ...
  if (d.state === "closed")
    await gh(`/repos/${OWNER}/${REPO}/issues/${created.number}`, {
      method: "PATCH",
      body: { state: "closed" },
    });
  state.pairs[card.id] = {
...
async function createCardFromIssue(iss, state) {
  if (!newCardListId) {
    console.warn(
      "   ⚠️  Sem lista de destino no board; pulando criação de card.",
    );
    return;
  }
  const d = desiredCard(iss);
  if (DRY_RUN) {
    console.log(`   [dry] criaria card da issue #${iss.number}`);
    return;
  }
  ...
  if (idLabels.length)
    await trello(`/cards/${card.id}`, { method: "PUT", params: { idLabels } });
  await trello(`/cards/${card.id}/attachments`, {
    method: "POST",
    params: { url: iss.html_url, name: `GitHub Issue #${iss.number}` },
  });
  if (d.state === "closed" && primaryDoneListId)
    await trello(`/cards/${card.id}`, {
      method: "PUT",
      params: { idList: primaryDoneListId },
    });
...
console.log(
  `🏁 Lista(s) tratada(s) como CONCLUÍDO (viram issue fechada): ${[...doneListIds].map((id) => `"${listNameById.get(id)}"`).join(" | ") || "⚠️ NENHUMA — nada será fechado! Confira TRELLO_DONE_LIST."}`,
);
console.log(
  `📥 Cards vindos de issues cairão na lista: "${listNameById.get(newCardListId) || "?"}"`,
);
...
for (const m of memberById.values()) {
  ...
  console.log(
    `   • @${m.username}${m.fullName ? ` (${m.fullName})` : ""} -> ${login ? `@${login} ✅` : "❌ sem correspondência (adicione ao MEMBER_MAP e convide como colaborador do repo)"}`,
  );
}
...
console.log(
  `🔁 ${cards.length} cards, ${allIssues.length} issues. Sincronizando…`,
);
...
for (const card of cards) {
  if (card.closed) {
    ...
    if (num && originOf.get(card.id) === "trello") {
      ...
      if (iss && iss.state !== "closed") {
        ...
        console.log(`   ↩︎ Issue #${num} fechada (card arquivado)`);
        changes++;
      }
    }
    continue;
  ...
  const num = cardToIssue.get(card.id);
  if (!num) {
    await createIssueFromCard(card, state);
    changes++;
    continue;
  }
  ...
  const iss = issueByNumber.get(num);
  if (!iss) {
    // issue sumiu -> recria a partir do card
    if (originOf.get(card.id) === "trello") {
      delete state.pairs[card.id];
      await createIssueFromCard(card, state);
      changes++;
    }
    continue;
  ...
  const st =
    ...
    (state.pairs[card.id] = {
      issue: num,
      origin: originOf.get(card.id) || "trello",
    });
  st.issue = num;
  ...
  if ((originOf.get(card.id) || st.origin) === "github") {
    ...
    const h = sha1(d);
    if (st.hash !== h) {
      await applyCardFromIssue(card, d);
      st.hash = h;
      changes++;
      console.log(`   ⬅︎ Card atualizado da issue #${num}`);
    }
  } else {
    const d = await desiredIssue(card);
    const h = sha1(d);
    if (st.hash !== h) {
      await applyIssueFromCard(iss, card, d);
      st.hash = h;
      changes++;
      console.log(`   ➡︎ Issue #${num} atualizada do card`);
    }
  }
}
...
if (MIRROR) {
  for (const iss of allIssues) {
    ...
    await createCardFromIssue(iss, state);
    changes++;
  }
}
...
console.log(
  changes
    ? `✨ Concluído. ${changes} alteração(ões).`
    : "✨ Concluído. Nada mudou.",
);
